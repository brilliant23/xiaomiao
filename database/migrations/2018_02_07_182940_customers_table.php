...
class CustomersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('customers', function (Blueprint $table) {
            $table->increments('id');
            $table->string('company_name',50)->comment('公司名称')->default('');;
            ...
            $table->timestamp('cooperate_time')->comment('合作时间')->useCurrent();
            $table->timestamp('get_business_time')->comment('下照日期')->useCurrent();;
            $table->timestamp('revenue_time')->comment('税务报道日期')->useCurrent();;
            $table->integer('account_id')->unsigned()->comment('负责会计人员')->default(0);
            $table->integer('sale_id')->unsigned()->comment('合作销售')->default(0);
            $table->integer('total_charge')->unsigned()->default(0);
            $table->integer('sale_charge')->unsigned()->default(0);
            $table->integer('last_charge')->unsigned()->default(0);
            $table->integer('one_charge')->unsigned()->default(0);
