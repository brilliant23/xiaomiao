                ...
                <div class="modal-body">
                    <form name="myForm" id="form1" novalidate>
                        <div class="form-group" ng-class="{ 'has-error' : !myForm.name.$pristine && myForm.name.$invalid }">
                            <label for="dept-name" class="control-label">名称:</label>
                            <input type="text" class="form-control" name="name"  id="dept-name" required ng-model="dept.name">
        ...
        angular.module('myModule', ['bsTable'])
            .controller('myController', function ($scope, $http) {
                ...
                $scope.btnquery = function () {
                    $("#table").bootstrapTable('refresh', {url: "{{route('dept.lists')}}"});
                ...
                $scope.save = function (modalstate, id) {
                    var url = "{{route('dept.store')}}";
                    var dataparam = $.param($scope.dept);
                    if (modalstate === 'edit') {
                        url += '/' + id;
